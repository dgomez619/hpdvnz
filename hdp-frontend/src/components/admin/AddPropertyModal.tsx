...
import { X, Plus, Search, Image as ImageIcon, Trash2 } from 'lucide-react';
...
export const AddPropertyModal = ({ isOpen, onClose, onSuccess, propertyToEdit }: ModalProps) => {
    const [formData, setFormData] = useState({
        ...
        baths: '',
        amenities: '',
        images: [] as string[] // Array to hold Cloudinary URLs
    });
    const [loading, setLoading] = useState(false);

    // CONFIGURATION (Fill these with your Cloudinary details)
    const CLOUD_NAME = 'dwrinmdz0'; 
    const API_KEY = '184389541388686'; // Solo necesario para la Media Library, no para el Upload Widget
        ...
        window.cloudinary.openMediaLibrary(
            ...
            },
            {
                insertHandler: (data: any) => {
                    const newUrls = data.assets.map((asset: any) => asset.secure_url);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = isEditing ? `http://localhost:5001/api/properties/${propertyToEdit?._id}` : 'http://localhost:5001/api/properties';
